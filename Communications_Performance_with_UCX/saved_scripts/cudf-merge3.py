...
async def send_df(ep, rank, dfs, bd_rec, frame_size, side):
    ...
    bd_rec.append((rank, "shead_1", side, *header_nbytes, time.time()))
...
async def recv_df(ep, rank, bd_rec, frame_size, side):
    ...
    for id in range(2):
        ...
        bd_rec.append((rank, "rframe", side, flen, time.time()))
        for frame in frames:
            await ep.recv(frame)
        rframepstart = time.time()
        bd_rec.append((rank, "rframe_1", side, flen, rframepstart))
