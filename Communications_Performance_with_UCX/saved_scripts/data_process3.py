...
def parse_args():
    ...
    parser.add_argument(
        ...
        "--len",
        type=str,
        help='prefix of length of local chunk'
    )
    parser.add_argument(
        "-d",
        "--device",
        ...
        if(mark[hashstr] == 0 and rec["event"] == event):
            ...
            if(tot_num != 0):
                ...
                if(event != "frame" and not event.startswith("tot")):
                    ...
                    if(event == "frame"):
                        if(rank1 // 4 == rank2 // 4):
                            len_rec[0].append(tot_len)
                        else:
                            len_rec[1].append(tot_len)
...
def main():
    ...
    process_tot(tot_rec)
    for id, it in enumerate(len_rec):
        it_cp = float(np.mean(np.asarray(it.copy())))
        print(args.len, len_name[id], it_cp)
